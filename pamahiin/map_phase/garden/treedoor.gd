...
@export var marking_id: String = ""        # ex: "wind", "spiral", etc.
...
# When GardenState updates correct trees
func _on_correct_markings_changed(new_set: Array) -> void:
	is_correct_tree = marking_id in new_set


# ZoneAController also calls this directly
...
func _process_knock() -> void:
	...
	# ---------------------------------------------------------
	# CORRECT KNOCK — correct tree + cicadas ON
	# ---------------------------------------------------------
...
# TRUNK SWAP HANDLER
# ---------------------------------------------------------
func _on_trunk_swap_triggered(step: int) -> void:
	return
